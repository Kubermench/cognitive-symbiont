...
class PubSubClient:
    ...
    def publish(self, event: Dict[str, Any]) -> None:
        ...
        if self.backend == "redis" and self._redis is not None:
            ...
            self._publish_with_retry(self._publish_file, payload)

    def _publish_redis(self, payload: str) -> None:
        """Publish to Redis without retry logic."""
        self._redis.publish(self.channel, payload)

    def _publish_file(self, payload: str) -> None:
        """Publish to file without retry logic."""
    ...
    def _publish_with_retry(self, publish_func, payload: str) -> None:
        ...
        try:
            ...
            logger.error("Failed to publish initiative event after %s attempts: %s", self.retry_attempts, exc)
